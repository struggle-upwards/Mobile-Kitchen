...
class ClubServiceMember extends BaseModel {

	public $not_null = '123';
	public $group_name = '';
    public function tableName() {
        return '{{gf_club_customer_service_group_member}}';
    }
    ...
    protected function afterFind() {
        parent::afterFind();
        if ($this->id != null) {
            $group=ClubServiceMember::model()->findBySql('select GROUP_CONCAT(g.group_name) as group_name from gf_club_customer_service_group g,gf_club_customer_service_group_member m  where find_in_set(g.id,m.group_id) and m.id=' . $this->id);
