...
abstract class CActiveRecord extends CModel
{
	 ...
	 * @since 1.1.7
	 */
	protected function query($criteria,$all=false,$checkResult=true)
	{
		$this->beforeFind();
		...
		if(empty($criteria->with))
		{
			...
			return $all ? $this->populateRecords($command->queryAll(), true, $criteria->index,$checkResult) : $this->populateRecord($command->queryRow(),true,$checkResult);
		}
		else
		{
	...
	public function find($condition='',$params=array(),$checkResult=true)
	{
		...
		return $this->query($criteria,false,$checkResult);
	}
	 ...
	 * @param mixed $condition query condition or criteria.
	 * @param array $params parameters to be bound to an SQL statement.
	 * @param boolean $checkResult Check whether the returned result is the table field
	...
	public function findAll($condition='',$params=array(),$checkResult=true)
	{
		...
		$criteria=$this->getCommandBuilder()->createCriteria($condition,$params);
		return $this->query($criteria,true,$checkResult);
	}
	...
	public function updateSQL($sql,$params=array())
	{
		Yii::trace(get_class($this).'.updateSQL()','system.db.ar.CActiveRecord');
		$builder=$this->getCommandBuilder();
	 ...
	 */
	public function populateRecord($attributes,$callAfterFind=true,$checkResult=true)
	{
		if(!$checkResult){
			return $attributes;
		}
	 ...
	 */
	public function populateRecords($data,$callAfterFind=true,$index=null,$checkResult=true)
	{

		$records=array();
		foreach($data as $attributes)
		{
			if(($record=$this->populateRecord($attributes,$callAfterFind,$checkResult))!==null)
			{
				if($index===null)
					$records[]=$record;
				else
