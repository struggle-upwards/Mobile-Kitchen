        ...
        <div>
            <form action="<?php echo Yii::app()->request->url;?>" method="get">
                ...
                <label style="margin-right:10px;">
                    <span>手机号：</span>
                    <input id="gfuser" style="width:200px;" class="input-text" type="text" name="keywords" value="<?php echo Yii::app()->request->getParam('keywords');?>" placeholder="请输入完整手机号">
                </label>
...
<?php foreach($arclist as $v) { ?>
    <?php if ($v->GF_ID) { ?>
                    <tr id="select" data-id="<?php echo $v->GF_ID; ?>" data-phone="<?php echo $v->security_phone; ?>" data-title="<?php echo $v->select_title; ?>" data-passed="<?php echo $v->passed; ?>" data-passed_name="<?php echo $v->passed_name; ?>" data-zsxm="<?php echo $v->ZSXM; ?>" data-sex="<?php echo $v->real_sex; ?>" data-card="<?php echo $v->id_card; ?>" data-birthday="<?php echo $v->real_birthday; ?>" data-security_phone="<?php echo $v->security_phone; ?>" data-native="<?php echo $v->native; ?>" data-nation="<?php echo $v->nation; ?>" style="<?php if($v->passed == 371 || $v->passed == 373) echo 'opacity: 0.2'; ?>">
                        <td width="30%"><?php echo $v->GF_NAME; ?></td>
                        <td width="35%"><?php echo $v->security_phone; ?></td>
                        <td width="35%">
                            <?php
                                if($v->passed == 136) {
                                    echo "未认证";
...
<script>
    ...
    $('#select').on('click',function(){
         window.location.href="<?php echo $this->createUrl('create'); ?>&passed=2&id="+$.dialog.data('GF_ID');
    })
</script>
