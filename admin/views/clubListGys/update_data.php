...
<div class="box">
    <div class="box-title c">
        <h1> <span>当前界面：供应商》
                》
                <?php
                if (@$_GET['action'] == 'index_prove_apply') {
                    echo '供应商认证》添加认证》添加';
                } else if (@$_GET['action'] == 'index_prove_wait_exam') {
                    echo '供应商认证》添加待审核》详情';
                } else if (@$_GET['action'] == 'index_prove_exam' || @$_GET['action'] == 'index_exam') {
                    echo '供应商认证》商家认证审核》详情';
                } else if (@$_GET['action'] == 'index_prove_cancel_nopass') {
                    echo '供应商认证》审核未通过》详情';
                } else if (@$_GET['action'] == 'index_prove_no_exam') {
                    echo '供应商认证》商家认证审核》待审核》详情';
                } else if (@$_GET['action'] == 'index_gys') {
                    echo '供应商管理》供应商列表》详情';
                } else if (@$_GET['action'] == '') {
                    echo '信息认证申请';
                }
    ...
    <div class="box-detail">
        ...
        <div class="box-detail-bd">
            <input value="<?php echo $_GET['id']; ?>" name="club_bianhao" type="hidden" />
            <div style="display:block;" class="box-detail-tab-item">
                <table id="t3" width="100%" style="table-layout:auto;">
                    <?php echo $form->hiddenField($model, 'club_type', array('value' => 380)); ?>
                ...
                <table id="t5" class="mt15" width="100%" style="table-layout: auto;">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            if ($model->edit_state == "" || $model->edit_state == 721 || $model->edit_state == 1538) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($model->edit_state == "" || $model->edit_state == 721 || $model->edit_state == 1538) { ?>
                                <script>
                ...
                <table id="t6" width="100%" style="table-layout:auto; margin-top:10px;">
                    ...
                    <tr>
                        ...
                        <td width="90%" colspan="3">
                            <?php
                            if ($model->edit_state != 721 && $model->edit_state != 1538 && !is_null($model->edit_state)) {
            ...
            <?php if ($model->edit_state == 721 || $model->edit_state == 1538 || is_null($model->edit_state)) { ?>
                <script>
        ...
        <table id="t8" class="mt15" width="100%" style=" table-layout:auto;">
            ...
            <tr>
                <td colspan="4">
                    ...
                    <?php echo $form->error($model, 'is_read', $htmlOptions = array()); ?>《<a target="_Blank" href="https://gw.gfinter.net/?device_type=7&c=info&a=page_switch&category=rule&page=supplier_service_protocol">供应商服务协议</a>》
                </td>
            </tr>
            <tr>
            ...
            <?php if((($_REQUEST['action']=='index_prove_no_exam'&&$model->edit_state==371)||$model->edit_state==2||$model->edit_state==373||$model->edit_state==1538)&&($_REQUEST['action']!='index_prove_apply')){?>
            <tr>
                <td width='10%'><?php echo $form->labelEx($model, 'edit_reasons_for_failure'); ?></td>
                <td width="90%" colspan="3">
                    <?php echo $form->textArea($model, 'edit_reasons_for_failure', array('class' => 'input-text','readonly'=>(!empty($_REQUEST['action'])&&$_REQUEST['action']!='index_prove_no_exam'?true:false))); ?>
                    <?php echo $form->error($model, 'edit_reasons_for_failure', $htmlOptions = array()); ?>
                </td>
            </tr>
            <?php }?>
            <?php if($_REQUEST['action']=='index_prove_apply'||$_REQUEST['action']=='index_prove_wait_exam'||$_REQUEST['action']=='index_prove_no_exam'){?>
            <tr>
                <td width="10%">可执行操作</td>
                <td width="90%" colspan="3">
                    <?php
                    if ($model->edit_state == 371) {
                        if (@$_GET['action'] == 'index_prove_wait_exam') {
                        ...
                        } elseif (@$_GET['action'] == 'index_prove_no_exam') {
                        ...
                        if($_REQUEST['action']=='index_prove_wait_exam'){
                            echo '<button id="baocun" onclick="submitType='."'baocun'".'" class="btn btn-blue" type="submit"> 保存</button>&nbsp;<button id="shenhe" onclick="submitType='."'shenhe'".'" class="btn btn-blue" type="submit"> 提交审核</button>&nbsp;<button class="btn" type="button" onclick="we.back();">取消</button>';
                        }else{
