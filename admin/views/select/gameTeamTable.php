<style>
    .list tr td:last-child,.list tr th:last-child{border-right: solid 1px #ddd;}
</style>
<div class="box">
            ...
            <table class="list" style="width: 49%;float:left;margin-right:2%;">
                ...
                <tbody>
                    <tr data-id="0" data-title="轮空">
                        <td class="click_td" style="color:red;" colspan="4">轮空</td>
                    </tr>
                    <?php
                        $index = 1;
                        foreach($arclist as $key => $v){
                            ...
                            // echo $num2.'***'.$key.'***'.$float;
                    ?>
                        <tr id="id_<?php echo $v->id; ?>" data-id="<?php echo $v->id; ?>" data-title="<?php echo $v->name; ?>" data-logo="<?php echo $v->logo; ?>">
                            <td class="click_td">
                                <?php
                            ...
                            <td class="click_td">
                                <?php
                                    if($v->game_list_data->game_player_team!=665 && $v->game_list_data->game_player_team!=666){
                                        ...
                                        echo $cf;
                                        echo '<script>$("#id_'.$v->id.'").attr("data-title","'.$cf.'");</script>';
                                    }
                                    else{
                                        echo $v->name;
                                    }
    ...
    $(function(){
        ...
        $('.box-table tbody tr .click_td').on('click', function(){
            ...
            if($.trim(thisa.children(".text:last-child").text())==''){
                ...
                var name=thisa.attr('data-title');
                var logo=thisa.attr('data-logo');
                $.dialog.data('id', id);
                $.dialog.data('name', name);
                $.dialog.data('logo', logo);
                $.dialog.close();
            }
        });
    });
