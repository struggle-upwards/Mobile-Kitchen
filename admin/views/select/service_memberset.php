...
<?php foreach($arclist as $v){ ?>
                    ...
                    <?php $value=GfPartnerMemberValues::model()->findAll('set_id='.$v->id.' AND set_input_id is NULL'); ?>
                    <?php if(isset($value) && !empty($value)){?>
                    <?php foreach($value as $v2){?>
                    <tr style="display:none;" class="item item_<?php echo $v->id; ?>" data-id="<?php echo $v->id; ?>" data-name="<?php echo $v->title; ?>" data-code="<?php echo $v->code; ?>" data-attrid="<?php echo $v2->id; ?>" data-attr="<?php echo $v2->attr_values; ?>/<?php echo $v2->attr_unit; ?>(<?php echo $v2->effective_date; ?>天)">
                        <td><?php echo $v2->attr_values; ?>/<?php echo $v2->attr_unit; ?>(<?php echo $v2->effective_date; ?>天)</td>
                    </tr>
                    <?php }?>
                    <?php }?>
<?php } ?>
