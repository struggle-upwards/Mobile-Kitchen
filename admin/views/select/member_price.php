        ...
        <div>
            <form action="<?php echo Yii::app()->request->url;?>" method="get">
                ...
                <label style="margin-right:20px;">
                    <span>销售类型：</span>
                    <?php echo downList(MallSaleName::model()->getSaleType(),'sale_id','sale_name','sale_type'); ?>
                </label>
...
$(function(){
    ...
    $('.box-table tbody tr').on('click', function(){
        var $this=$(this);
        $.dialog.data('member_price_id', $this.attr('data-id'));
        $.dialog.data('member_price_name', $this.attr('data-name'));
        $.dialog.data('member_price_code', $this.attr('data-code'));
        $.dialog.close();
    });
});
</script>