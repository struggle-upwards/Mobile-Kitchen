...
<div class="box">
    <div class="box-title c"><h1><i class="fa fa-table"></i>GF靓号列表</h1></div><!--box-title end-->
    <div class="box-content">
        <div class="box-header">
            <a class="btn" href="javascript:;" onclick="we.reload();"><i class="fa fa-refresh"></i>刷新</a>
            <a style="display:none;" id="j-delete" class="btn" href="javascript:;" onclick="cancel('.check-item input:checked',1);">取消保留</a>
        </div><!--box-header end-->
        <div class="box-search">
            <form action="<?php echo Yii::app()->request->url;?>" method="get">
                <input type="hidden" name="r" value="<?php echo Yii::app()->request->getParam('r');?>">
                <!-- <a id="j-check" class="btn" href="javascript:;" onclick="cancel('.check-item input:checked',1);" style="vertical-align: middle;margin-top: 10px;margin-right:10px;">一键取消保留</a> -->
                ...
                <?php $index = 1;
                 foreach($arclist as $v){ ?>
                    <tr>
                        ...
                        <td style="text-align: center"><?php echo $v->f_vip==1?'保留号':''; ?></td>
                        <td style='text-align: center;'>
                            <a class="btn" onclick="cancel(<?php echo $v->id;?>);" href="javascript:;" title="取消保留">取消保留</a>
                        </td>
                    </tr>
                <?php $index++; } ?>
                </tbody>
...
<script>
    var deleteUrl = '<?php echo $this->createUrl('cancel', array('id'=>'ID','al'=>'取消保留成功'));?>';
    ...
    function cancel(id,val){
        ...
        var can1 = function() {
            $.ajax({
                type: 'get',
                url: '<?php echo $this->createUrl('cancel');?>&id='+id+'&al=取消保留成功',
                // data: {id: id},
                dataType: 'json',
