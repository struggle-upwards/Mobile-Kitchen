<div class="box">
    ...
    <div class="box-content">
        <div class="box-header">
            <!-- <a class="btn" onclick="add_member()"><i class="fa fa-plus"></i>添加注销</a> -->
            <a class="btn" href="index?r=gfUserLock/create&action=shutdown">注销</a>
        </div><!--box-header end-->
            ...
            <table class="list">
                <thead>
                    <tr>
                        <th width="3%">序号</th>
                        <th width="12%">GF账号</th>
                        <th width="12%"><?php echo $model->getAttributeLabel('GF_NAME');?></th>
                        <th width="12%"><?php echo $model->getAttributeLabel('user_state');?></th>
                        <th width="12%">注销原因</th>
                        <th width="12%">注销状态</th>
                        <th width="12%">申请时间</th>
                        <th width="12%"><?php echo $model->getAttributeLabel('admin_gfid');?></th>
                        <th width="12%">操作</th>
                    </tr>
                </thead>
                <tbody>
...
?>
                    <tr>
                        ...
                        <td><?php echo (!empty($v->state_name)) ? $v->state_name : $v->state; ?></td>
                        <td><?php echo $v->uDate; ?></td>
                        <td><?php echo $v->admin_gfname; ?></td>
                        <td>
                            <?php
                                echo show_command('详情',$this->createUrl('update', array('id' => $v->ID, 'action' => 'shutdown'))).'&nbsp;&nbsp;';
                                if($v->state == 371) {
                                    echo '<a class="btn" href="javascript:;" onclick="clickRollback('.$v->ID.')">撤销</a>';
                                }else {
...
<script>
    ...
    // 撤销申请
    function clickRollback(id,state=374) {
        $.ajax({
            type: 'post',
            url: '<?php echo $this->createUrl('Rollback'); ?>&id='+id+'&state=374',
            dataType: 'json',
            success: function(data){
                console.log(data);
                if (data.status==1){
