<?php
    ...
    if($_REQUEST['passed'] == 136) {
        $txt = '注册账号';
    } else if($_REQUEST['passed'] == 372 && empty($model->GF_ID)) {
        $txt = '注册实名';
    } // else if($_REQUEST['passed'] == 372 && !empty($model->GF_ID)) {
        // $txt = '账号实名';
    // }
   ...
   <div class="box-detail">
        <!-- <div class="box-detail-tab" style="margin-top:10px;">
            <ul class="c">
                ...
                <li class="<?//=$_REQUEST['passed']==372?'current':'';?>"><a href="<?php //echo $this->createUrl('create',array('passed'=>372));?>">实名注册</a></li>
            </ul>
        </div> -->
            ...
            <div style="display:block;" class="box-detail-tab-item">
                <table width="100%" class="mt15" style="table-layout:auto;">
                    <tr class="table-title">
                        <td colspan="4"><?= $_REQUEST['passed'] == 136 ? '注册账号信息' : $_REQUEST['passed'] == 372 && ($_REQUEST['action'] == 'realname') ? '账号实名' : '注册账号';?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td width="85%" id="df_gf_name">
                            <?php if($_REQUEST['action'] == 'realname') {
                                ...
                                echo $form->textField($model, 'GF_NAME', array('class' => 'input-text','onChange' =>'uniqueName(this)','autocomplete'=>'off','readonly'=>true));
                            } ?>
                            <?php echo $form->error($model, 'GF_NAME', $htmlOptions = array()); ?>
                        </td>
                    </tr>
                    <?php if(empty($model->GF_ID)){;?>
                    <?php // if($_REQUEST['passed']==372){?>
                        <!-- <tr> -->
                            <!-- <td><?php // echo $form->labelEx($model, 'security_phone'); ?></td> -->
                            <!-- <td>
                                <?php // echo $form->textField($model, 'security_phone', array('class' => 'input-text')); ?>
                    ...
                    <?php // }?>
                        <?php if($_REQUEST['action'] != 'realname') { ?>
                            <tr>
                                ...
                                <td>
                                    <?php echo $form->passwordField($model, 'GF_PASS', array('class' => 'input-text','readonly'=>true)); ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php echo $form->passwordField($model, 'GF_PASS2', array('class' => 'input-text','readonly'=>true)); ?>
...
<script>
    $("input[type='password'],#GfUser1_GF_NAME").on("focus",function(){
        $(this).removeAttr('readonly');
    })
    $("input[type='password'],#GfUser1_GF_NAME").on("blur",function(){
    ...
    $('#check').on('click', function(){
        ...
        close: function () {
            if($.dialog.data('id')>0){
                $('#GfUser1_GF_ACCOUNT').val($.dialog.data('account'));
                // $('#GfUser1_GF_ID').val($.dialog.data('id'));
        ...
        $.dialog.data('GF_ID', 0);
            ...
            close: function () {
                if($.dialog.data('GF_ID')>0){
                    $('#GfUser1_GF_ACCOUNT').val($.dialog.data('GF_ACCOUNT'));
                    $('#GfUser1_GF_ID').val($.dialog.data('GF_ID'));
                    $account_box.html('<span class="label-box">' + $.dialog.data('GF_ACCOUNT') + '</span>');
                    $('#GfUser1_GF_NAME').val($.dialog.data('GF_NAME'));
    ...
    function reRealname(id) {
        ...
        $('#reloadRN').load('"index?r=gfUser1/update_realname&id="+id');
        // window.location.href("index?r=gfUser1/update_realname&id="+id);
    }
    ...
    $('#GfUser1_id_card_validity_start').on('click', function(){
        WdatePicker({startDate:'%y-%M-%D 00:00:00',dateFmt:'yyyy-MM-dd'});});
    $('#GfUser1_id_card_validity_end').on('click', function(){
        WdatePicker({startDate:'%y-%M-%D 00:00:00',dateFmt:'yyyy-MM-dd'});});

    $("#GfUser1_id_card_type").on("change",function(){
    ...
    $("#tongguo").on("click",function(){
        if($("#GfUser1_id_card_type").val()==''){
            we.msg('minus', '请选择证件类型');
            return false;
        }
        if($("#GfUser1_id_card_type").val()==843||$("#GfUser1_id_card_type").val()==845){
            if($("#GfUser1_id_card_pic").val()==''){
                we.msg('minus', '请上传证件正面');
                return false;
            }
            if($("#GfUser1_id_pic").val()==''){
