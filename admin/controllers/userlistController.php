...
class userlistController extends BaseController {
    ...
    public function actionIndex(
            $keywords='', $passed='',
            $user_state='',$realname_time='',
            $realname_entertime='',$real_sex='',
            $country='',$province='',
            $to_day=0, $logon_way=''
    ) {
        ...
        $REGTIME = '';
        $criteria->condition = 'passed in(136,371,2,373)';
        if($to_day==1){
        ...
        $data['total_count'] = $model->count($criteria);
        $data['passed'] = BaseCode::model()->getReturn('136,371,2,373');
    ...
    public function actionIndex_real_name($valid_date='',$end_valid_date='',$keywords='',$nation='',$real_sex='',$id_card_type='',$state=0) {
        ...
        $criteria->condition = 'user_state in(506,507,1282,1283) and passed=2';
        if($state==1){
            // $criteria->condition = 'user_state=506 and passed=2 and left(realname_entertime,10)='.date("Y-m-d");
        ...
        $data['id_card_type'] = BaseCode::model()->getCode(842);
        $data['country_name'] = TCountry::model()->findAll('is_visible=649');
        $data['count1'] = $model->count('user_state=506 and passed=2 and left(realname_entertime,10)="'.date("Y-m-d").'"');
    ...
    public function actionCreate($id=0) {
        $modelName = $this->model;
        $model = (($id==0)?new $modelName('create'):$this->loadModel($id));
        $data = array();
        if (!Yii::app()->request->isPostRequest) {
            ...
            //注册方式为 后台添加
            $model->logon_way="后台添加";
            $model->logon_way_name="后台添加";
            //$model->user_state=506;//不知道用户类型怎么回事，先屏蔽
    ...
    public function actionExist($name=0) {
        $count=userlist::model()->count('GF_NAME="'.$name.'"');
        if($count>0) {
 ...
 //        if($model->passed==2&&$_POST['submitType']=='tongguo'){
 //            if(empty($model->GF_ID)){
 //                $qp=userlist::model()->find("id_card_type=".$model->id_card_type." and id_card='".$model->id_card."'");
 //            }else{
 //                $qp=userlist::model()->find("GF_ID<>".$model->GF_ID." and id_card_type=".$model->id_card_type." and id_card='".$model->id_card."'");
 //            }
 //        }
 //        if(!empty($qp)){
 //            $st=0;
 ...
 //    }
    ...
    function saveData($model,$post) {
        ...
        //put_msg($post);
        if(isset($post['GF_PASS'])){
            //密码小于6位
            if(mb_strlen($post['GF_PASS'])<6){
                show_status(0,'',returnList(),'密码小于6位数，请输入长一些的密码');
                return;
            }
            //两次密码不一致
            if($post['GF_PASS']!=$post['GF_PASS2']){
                show_status(0,'',returnList(),'两次密码不一致');
                return;
            }
        }
        //（阿里云身份证验证）：如果有ZSXM与id_card但是passed==136就进行验证
        if($post['ZSXM']&&$post['id_card']&&($model->passed==136||!$model->passed)){
            $res=$model->readlNameAuthen($post['ZSXM'],$post['id_card']);
            if($res['respCode']!='0000'){
                show_status(0,'','','实名认证失败,'.$res['respMessage']);
                return;
            }
        }
        //更新
        $model->modelUpdate($post);
    }
    ...
    public function  actionTothaw(){
        ...
        foreach ($GF_IDArray as $v1){
            $user = userlist::model()->find('GF_ID='.$v1);
            $user->user_state=506;
            if ($thawMode == 0){
    ...
    public function actionwxLogin($code,$phone_code=""){
        $openid=userlist::model()->getwxOpenid($code);
        //
        $user=userlist::model()->find('openid="'.$openid.'"');
        //没有注册就注册一下
        if(empty($user)&&$phone_code){
            $user = userlist::model()->newWxUser($openid,$phone_code);
        }
        $res=$user->getWxInfo();
    ...
    public function actionUploadHeadPic(){
        ...
        {
            $model=userlist::model()->find('openid="'.$openid.'"');
    ...
    public function actionupdateRole($id){
        ...
        if(!Yii::app()->request->isPostRequest){
            ...
            if(empty($temp)){
                show_status(0,'',returnList(),'标识码为'.$code.'的'.$roles[$roleCode].'不存在');
                return;
            }
            $msg = array(
                'staCode'=>'',
            ...
            if(!empty($check)){
                if($check->state==371||$check->state==2)
                    show_status(0,'添加成功',returnList(),'已有相同申请，请检查审核列表或审核记录');
                return;
            }
            show_status($user->claimRole($msg),'添加成功',returnList(),'添加失败');
        }
    }
    //获取用户的角色申请历史 小程序接口
