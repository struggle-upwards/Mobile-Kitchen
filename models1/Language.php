...
public function getSwitch(){
    $rs=array();
    //  $data['send_mail_on']='off';
    $rs['send_mail_on']='off';
    $rs['goods_on']='off';//on
    $rs['order_on']='off';//on
    $rs['ads_on']='off';//on
    $rs['comment_manage_on']='off';//on
    $data['goods_sum']=0;
    $data['order_sum']=0;
    $data['ads_sum']=0;
    $data['comment_sum']=0;
...
public function getLang($pname) {
      $rs= $this->getName($pname);
      return (!empty($rs)) ? $rs : $this->getName2($pname);
}
public function getName($pname){
$_LANG=array();        
$_LANG['shop_guide'] = '注册单位向导';
...
$_LANG['before_team_member'] = '本系统贡献者';
/*后台语言项*/
$_LANG['send_mail_off'] = '自动发送邮件关闭';
 return isset($_LANG[$pname]) ? $_LANG[$pname] :'';
}
public function getName2($pname){
$_LANG=array();
